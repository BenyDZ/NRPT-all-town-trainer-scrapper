...
def getTownName(townUrl,townNames):
    #condition to read each element of townUrl
    for url in townUrl:
        #add url text in townNames
        townNames.append(url.text)
        #return townNames
    return townNames
...
def getTownUrl():
    ...
    result = getPage(page)
    #create the object that will contain all town href, return the object townHref of type list
    townHref = []
    #create the object that will contain all townName, return the object townName of type list
    townNames = []
    #create a beautifumsoup object, return the object soup of type bs4.BeautifulSoup
    soup = BeautifulSoup(result,features="html.parser")
    #get result container, return the object resultContainer of type list
    resultContainer = soup.find('div', class_="content").find_all('p')
    #initialise the object counter to 7
    counter = 7
    #condition to verifie if counter is less than 10
    while counter < 10:
        #get all link in the current container, return the object townUrl of type list
        townUrl = resultContainer[counter].find_all('a')
        #condition to read each element of townUrl
        for url in townUrl:
            #add all href in townHref
            townHref.append(url.get('href'))
        #initialise the object townName with the result of the function getTownName, return the object townName of type list
        townName = getTownName(townUrl,townNames)
        #increment counter to 1
        counter += 1
    #initialise the object townNameAndHref with townName and townHref, return the object townName and townHref of type list
    towNameAndHref = [townName,townHref]
    #return towNameAndHref
    return towNameAndHref

def getPaginationLinks ():
    #get the result of the research, return the object result of type bytes
    result = getPage(townHref)
    #create a beautifumsoup object, return the object soup of type bs4.BeautifulSoup
    soup = BeautifulSoup(result,features="html.parser")
...
def whriteInOuput(finalOutput):
    ...
    os.chdir("D:/IIHT/Python/Project/NRPT all town trainer scrapper/caches")
     #open text file, return  an object of type io.TextIOWrapper
    with open("{}.txt".format(townName), "w") as writ:
        #write each line in the object op, return an object of type int
        writ.write('\n'.join(finalOutput) + "\n")

towNameAndHref = getTownUrl()

for townName,townHref in zip(towNameAndHref[0],towNameAndHref[1]):
